...
@export_range(0, 360) var angle_deg = 360 : set = _set_angle_deg
...
@export var ray_count = 100 : set = _set_ray_count
...
func _set_angle_deg(new_value: float):
	angle_deg = new_value
	_angle = deg_to_rad(new_value)
	
	_angle_half = _angle / 2.
	_angular_delta = _angle / ray_count

func _set_ray_count(new_value: int):
	ray_count = new_value
	_angular_delta = _angle / ray_count
